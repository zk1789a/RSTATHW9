...
Salaries %>% count(playerID) %>% filter(n>1) %>% head()
Batting %>% count(playerID) %>% filter(n>1) %>% head()
```
...
```{r}
flights3 <- flights2 %>% left_join(airports, c("dest" = "faa")) %>% rename(c("dest_name" = "name.y"), c("dest." = "origin"), c("carrier_name" = "name.x"))
...
```{r}
flights3 <- flights3 %>% group_by(dest) %>% summarize(avg_delay = mean(dep_delay, na.rm = TRUE)) %>% select(everything(), avg_delay)  

flights4 <- flights3 %>% full_join(airports, c("dest" = "faa"))

flights4 <- flights4 %>% na.omit()

flights4 %>% ggplot(aes(lon, lat, color = avg_delay)) + borders("state") + geom_point() + coord_quickmap()
```   
